...
import {
    Store, Camera, Clock, Phone, Mail, Globe, MapPin,
    Plus, X, Save, Loader2, CheckCircle, Edit3, Star,
...
const CITY_OPTIONS = [
    ...
    'Bandung', 'Surabaya', 'Yogyakarta', 'Semarang', 'Medan', 'Makassar', 'Tangerang', 'Bekasi', 'Depok'
]

const FEATURE_OPTIONS = [
    'Garansi Resmi', 'Teknisi Bersertifikat', 'Spare Part Original', 'Free Konsultasi',
    'Home Service', 'Express Service', 'Pickup & Delivery', '24 Jam', 'Pembayaran Cicilan'
]
        ...
        <div className="min-h-screen">
            ...
            <div className="mb-8">
                <div className="flex items-center justify-between">
                    <div>
                        <h1 className="text-3xl font-bold text-gray-900">Kelola Profil Bisnis</h1>
                        <p className="mt-1 text-gray-600">Lengkapi profil untuk tampil di halaman rekomendasi</p>
                    </div>
                    <button
            ...
            <div className="mb-8 overflow-hidden rounded-2xl bg-gradient-to-r from-blue-600 via-cyan-600 to-blue-700 p-6 shadow-xl">
                <div className="flex items-center justify-between">
                    <div>
                        ...
                        <p className="mt-1 text-blue-100">Lengkapi semua informasi agar muncul di halaman rekomendasi</p>
                    </div>
                    <div className="flex items-center gap-4">
                        <div className="relative h-20 w-20">
                            <svg className="h-full w-full -rotate-90 transform">
                                <circle cx="40" cy="40" r="35" fill="none" stroke="rgba(255,255,255,0.2)" strokeWidth="8" />
                                <circle
                                    cx="40" cy="40" r="35" fill="none" stroke="white" strokeWidth="8"
            ...
            <div className="mb-6 flex gap-2 overflow-x-auto">
                {[
                    ...
                    <button
                        ...
                        className={`flex items-center gap-2 whitespace-nowrap rounded-xl px-5 py-3 font-medium transition-all ${activeTab === tab.id
            ...
            <div className="rounded-2xl border border-gray-200 bg-white p-6 shadow-sm">
                ...
                {activeTab === 'info' && (
                    <div className="space-y-6">
                        ...
                        {/* Banner Upload */}
                        <div>
                            <label className="mb-2 block text-sm font-medium text-gray-700">Banner Toko</label>
                            <div className="relative h-48 overflow-hidden rounded-2xl border-2 border-dashed border-gray-300 bg-gray-50">
                                {profile.banner ? (
                                    ...
                                    <div className="flex h-full flex-col items-center justify-center">
                                        ...
                                        <p className="text-gray-500">Upload banner toko (1920x600 recommended)</p>
                                        <input
                                            type="text"
                                            ...
                                            onKeyDown={(e) => {
                                                if (e.key === 'Enter') {
                                                    setProfile({ ...profile, banner: (e.target as HTMLInputElement).value })
                                                }
                                            }}
                                        />
                                    </div>
                        ...
                        <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
                            <div>
                                <label className="mb-2 block text-sm font-medium text-gray-700">Nama Toko</label>
                                <input
                                    type="text"
                                    value={profile.name}
                        ...
                        <div>
                            <label className="mb-2 block text-sm font-medium text-gray-700">Deskripsi Toko</label>
                            <textarea
                        ...
                        {/* Features */}
                        <div>
                            <label className="mb-3 block text-sm font-medium text-gray-700">Keunggulan Toko</label>
                            <div className="flex flex-wrap gap-2">
                                {FEATURE_OPTIONS.map((feature) => (
                                    <button
                                        key={feature}
                                        onClick={() => toggleFeature(feature)}
                                        className={`rounded-full px-4 py-2 text-sm font-medium transition-all ${profile.features.includes(feature)
                                                ? 'bg-blue-600 text-white'
                                                : 'bg-gray-100 text-gray-700 hover:bg-gray-200'
                                            }`}
                                    >
                                        {profile.features.includes(feature) && <CheckCircle className="mr-1 inline h-4 w-4" />}
                                        {feature}
                                    </button>
                                ))}
                            </div>
                ...
                {activeTab === 'services' && (
                    <div className="space-y-6">
                        <h3 className="text-xl font-bold text-gray-900">Layanan yang Ditawarkan</h3>
                            ...
                            {profile.services.map((service, index) => (
                                ...
                                >
                                    <button
                                        ...
                                        className="absolute right-2 top-2 rounded-full bg-red-100 p-1 text-red-600 opacity-0 transition-all group-hover:opacity-100 hover:bg-red-600 hover:text-white"
                                    >
                                        <X className="h-4 w-4" />
                                    </button>
                ...
                {activeTab === 'gallery' && (
                    <div className="space-y-6">
                        ...
                        <p className="text-sm text-gray-500">
                            Maksimal 8 foto. Gunakan foto berkualitas tinggi untuk hasil terbaik.
                        </p>
                        ...
                        <div className="grid grid-cols-2 gap-4 md:grid-cols-4">
                            {profile.gallery.map((image, index) => (
                                <div key={index} className="group relative overflow-hidden rounded-xl border border-gray-200">
                                    <img src={image} alt={`Gallery ${index + 1}`} className="h-40 w-full object-cover" />
                                    <button
                                        onClick={() => removeGalleryImage(index)}
                                        className="absolute right-2 top-2 rounded-full bg-red-600 p-2 text-white opacity-0 transition-all group-hover:opacity-100 hover:bg-red-700"
                                    >
                ...
                {activeTab === 'contact' && (
                    <div className="space-y-6">
                        <h3 className="text-xl font-bold text-gray-900">Kontak & Jam Operasional</h3>
                        ...
                        <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
                            ...
                            <div>
                                ...
                                <input
                                    ...
                                    onChange={(e) => setProfile({ ...profile, hours: { ...profile.hours, weekday: e.target.value } })}
                            ...
                            <div className="md:col-span-2">
                                ...
                                <input
                                    ...
                                    onChange={(e) => setProfile({ ...profile, hours: { ...profile.hours, weekend: e.target.value } })}
